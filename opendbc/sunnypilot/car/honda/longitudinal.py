...
def load_gas_alpha(car_fingerprint: str) -> float:
  try:
    raw_alpha = Params().get("HondaGasAlphaParams")
    ...
    alpha = float(raw_alpha.decode("utf-8") if isinstance(raw_alpha, bytes) else raw_alpha)
    return LongGasLearner._safe_gasalpha(alpha)
  except (OSError, json.JSONDecodeError, KeyError, TypeError, ValueError):
    return _GAS_ALPHA_LO
