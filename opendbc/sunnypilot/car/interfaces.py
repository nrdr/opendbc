...
from opendbc.car.honda.values import HondaFlags, HondaSafetyFlags
...
def _initialize_honda(CP: structs.CarParams, CP_SP: structs.CarParamsSP, params_dict: dict[str, str]) -> None:
  if CP.brand == 'honda':
    ...
    if honda_stock_long:
      ...
      # Honda's base interface derives the panda longitudinal safety bit before Sunny's force-stock
      # override runs. Clear that bit while preserving its already-decided radar availability.
