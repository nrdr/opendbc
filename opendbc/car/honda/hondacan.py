...
def create_brake_command(packer, CAN, apply_brake, pump_on, pcm_override, pcm_cancel_cmd, fcw, stock_brake, CP):
  ...
  if CP.flags & HondaFlags.HYBRID:
    values.update(COMPUTER_BRAKE_HYBRID=apply_brake, BRAKE_PUMP_REQUEST_HYBRID=apply_brake > 0)
  else:
    values.update(COMPUTER_BRAKE=apply_brake, BRAKE_PUMP_REQUEST=pump_on)
